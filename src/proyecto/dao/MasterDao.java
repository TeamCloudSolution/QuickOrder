...
public class MasterDao implements Serializable {
	...
	public List findAll(Class clase) throws Exception {
		Query query = entityManager.createNamedQuery(clase.getSimpleName() + ".findAll", clase);
		return query.getResultList();
	}

	@SuppressWarnings({ "rawtypes", "unchecked" })
	public List findAllQuery(Class clase, String sql, Parametro p) throws Exception {
	...
	public List findAllNativeQuery(String sql, Parametro p) throws Exception {
		Map<String, Object> parametros = p != null ? p.getParametros() : null;
		try {
	...
	@SuppressWarnings({ "rawtypes" })
	public List findAllNativeQuery(Class clase, String sql, Parametro p) throws Exception {
		Map<String, Object> parametros = p != null ? p.getParametros() : null;
		try {
	...
	public List findAllNamedQuery(Class clase, String namedQuery, Parametro p) throws Exception {
		Query query = entityManager.createNamedQuery(namedQuery, clase);
		if (p != null) {
			Map<String, Object> parametros = p.getParametros();
			List<String> keys = new ArrayList<String>(parametros.keySet());
			for (String key : keys) {
				query.setParameter(key, parametros.get(key));
			}
